...
# 레이트 리밋 설정 (비용/남용 방지)
# 일일 제한(초 단위 윈도우: 24시간), 버스트(1분)
RATE_WINDOW_SECONDS = int(os.getenv("RATE_WINDOW_SECONDS", 86400))
BURST_WINDOW_SECONDS = int(os.getenv("BURST_WINDOW_SECONDS", 60))
# 엔드포인트별 기본 제한 수치 (세션 또는 IP 기준)
SEARCH_DAILY_LIMIT = int(os.getenv("SEARCH_DAILY_LIMIT", 300))
SEARCH_BURST_LIMIT = int(os.getenv("SEARCH_BURST_LIMIT", 30))
UPLOAD_DAILY_LIMIT = int(os.getenv("UPLOAD_DAILY_LIMIT", 30))
ADDTEXT_DAILY_LIMIT = int(os.getenv("ADDTEXT_DAILY_LIMIT", 30))
